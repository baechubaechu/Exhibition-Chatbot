...
6. `npm run dev` → http://localhost:3000  
   - 화면이 **500 / Internal Server Error** 로만 나오면: **`npm run dev:clean`** (`.next` 삭제 후 dev) 또는 **`npm run build:clean`**. 기본은 Turbopack 없이 `dev`만 쓰는 것을 권장합니다(`dev:turbo`는 Windows에서 캐시 이슈가 날 수 있음).
